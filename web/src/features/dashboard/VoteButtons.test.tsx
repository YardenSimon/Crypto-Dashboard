...
vi.mock('@/lib/api', () => ({
  api: {
    getDashboard: vi.fn(),
    postVote: vi.fn(() => Promise.reject(new Error('Vote failed'))),
  },
}))
...
describe('VoteButtons optimistic update and rollback', () => {
  ...
  it('applies optimistic vote then rolls back on error', async () => {
    ...
    expect(upvoteBtn).not.toHaveClass('text-green-600')

    await userEvent.click(upvoteBtn)

    await waitFor(() => expect(upvoteBtn).toHaveClass('text-green-600'))

    await waitFor(() => expect(upvoteBtn).not.toHaveClass('text-green-600'))
  })
})
